...
public class Board {
	...
	public long getMillisecondsCounter()
	{
		return millisecondsCounter;
	}

	public void setMillisecondsCounter(long counter)
	{
		millisecondsCounter = counter;
	}
			...
			for(int x=0; x<width; x++){
				...
				if(b.isOpened() && b.getBombsSurrounding()>0)
				{
					if (flagsSurrounding(x,y)== b.getBombsSurrounding()&&unopenedAndUnflaggedAround(x, y)
						...
						hintColorYellow = flagsSurrounding(x, y) == b.getBombsSurrounding() && unopenedAndUnflaggedAround(x, y);
					}
				}
			}
		}
	...
	public void checkWin()
	{
		...
		if(getOpenedBoxCount() == totalBoxes - totalBombs)
		{
			...
			millisecondsCounter = System.currentTimeMillis() - millisecondsStart;
			game.updateScores((timeCounter*1000)+millisecondsCounter);
			System.out.println("*!*!* " +timeCounter*1000+ millisecondsCounter + "*!*!*");
	...
	public void paintBoard(Canvas g){
		...
		//PLUS
		g.drawLine((MainActivity.screenWidth*9)/10, realBarHeight+realBarHeight/22, (MainActivity.screenWidth*9)/10, (float) (realBarHeight+realBarHeight/14.69), paint);
		//MINUS
		...
		clockHandX1 = (float) (xOrig + (MainActivity.screenWidth/30.5)*Math.cos(theta1));
		clockHandY1 = (float) (yOrig + (MainActivity.screenWidth/30.5)*Math.sin(theta1));
		...
		else if(timeCounter<10)
		{
			...
			if(timeCounter < 0 )
				g.drawText(""+0, sX+(bSize)/3+((MainActivity.screenWidth)/18)+push, (float) (sY+(bSize*3)/4+((MainActivity.screenWidth)/101.4)), paint); //count
			else
				g.drawText(""+timeCounter, sX+(bSize)/3+((MainActivity.screenWidth)/18)+push, (float) (sY+(bSize*3)/4+((MainActivity.screenWidth)/101.4)), paint); //count
		}
